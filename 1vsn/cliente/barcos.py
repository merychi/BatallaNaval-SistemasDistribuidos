...
class Barco:
    def __init__(
        self, nombre_tipo, tam_casillas, sprite_path_h=None, sprite_path_v=None
    ):
        ...
        self.segmentos_tocados = [
            False
        ] * tam_casillas  # Estado de impacto por segmento
        self.sentido_actual = "h"  # Orientación: horizontal por defecto
        self.esta_colocado = False  # Indica si está colocado en el tablero
        self.tablero_logico = None  # Referencia al tablero lógico del jugador

        self.fila_inicial = -1  # Default to -1 indicating not placed
        self.columna_inicial = -1
        self.orientacion_colocacion = (
            ""  # To store the actual orientation used for placement
        )
        ...
        if sprite_path_h:
            try:
                self.sprite_original_h = pygame.image.load(
                    sprite_path_h
                ).convert_alpha()
            except pygame.error as e:
                print(
                    f"Error al cargar sprite horizontal '{sprite_path_h}' para {self.nombre_tipo}: {e}"
                )
        ...
        if sprite_path_v:
            try:
                self.sprite_original_v = pygame.image.load(
                    sprite_path_v
                ).convert_alpha()
            except pygame.error as e:
                print(
                    f"Error al cargar sprite vertical '{sprite_path_v}' para {self.nombre_tipo}: {e}"
                )
                self.sprite_original_v = None
        elif self.sprite_original_h:
            try:
                self.sprite_original_v = pygame.transform.rotate(
                    self.sprite_original_h, 90
                )
            except pygame.error as e:
    ...
    def _preparar_sprite_activo(self, ancho_celda_px):
        ...
        if self.sentido_actual == "h" and self.sprite_original_h:
            try:
                ...
                self.sprite_escalado_activo = pygame.transform.scale(
                    self.sprite_original_h, (int(nuevo_ancho), int(nuevo_alto))
                )
            except pygame.error as e:
                print(
                    f"Error al escalar sprite horizontal para {self.nombre_tipo}: {e}"
                )
            ...
            try:
                ...
                self.sprite_escalado_activo = pygame.transform.scale(
                    self.sprite_original_v, (int(nuevo_ancho), int(nuevo_alto))
                )
            except pygame.error as e:
                print(f"Error al escalar sprite vertical para {self.nombre_tipo}: {e}")

    def colocar_logicamente_y_preparar_sprite(
        self,
        fila_inicio,
        col_inicio,
        sentido,
        ...
        self.tablero_logico = tablero_logico_matriz  # Guardar referencia al tablero
        ...
        for i in range(self.tam_casillas):
            ...
            if not (0 <= r_actual < FILAS and 0 <= c_actual < COLUMNAS):
                print(
                    f"Error: '{self.nombre_tipo}' fuera de límites en ({r_actual},{c_actual})"
                )
            ...
            if tablero_logico_matriz[r_actual][c_actual] == "B":
                print(
                    f"Error: Superposición de '{self.nombre_tipo}' en ({r_actual},{c_actual})"
                )
                self.esta_colocado = False
                return False
        ...
        self.fila_inicial = fila_inicio
        self.columna_inicial = col_inicio
        self.orientacion_colocacion = sentido

        # Preparar sprite para renderizado
        self._preparar_sprite_activo(ancho_celda_px)
        if self.sprite_escalado_activo:
            self.posicion_renderizado_sprite = (
                offset_x_tablero_px + col_inicio * ancho_celda_px,
    ...
    def registrar_impacto(self, fila_impacto, col_impacto):
        ...
        if pos_impacto in self.posiciones_en_tablero:
            ...
            if not self.segmentos_tocados[idx]:
                self.segmentos_tocado[idx] = True
                print(
                    f"Barco '{self.nombre_tipo}' impactado en segmento {idx} en {pos_impacto}"
                )
            return True
        return False
    ...
    def esta_hundido(self):
        ...
        # Verificar que todos los segmentos estén impactados
        if not all(self.segmentos_tocados):
            return False
            
        # Verificación adicional contra el tablero lógico
        for r, c in self.posiciones_en_tablero:
            if self.tablero_logico[r][c] != "X":  # 'X' representa un impacto
                return False
